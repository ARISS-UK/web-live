...
$(function() {
    streamUrl = "/"+streamName+"/index.m3u8";
    plUrl = hlsSource + streamUrl;
    ...
    if(clientMobile)
    {
        ...
        statusText_active = "Stream Active - Tap to play!";
    }
    ...
    $.ajax({
        ...
        success: function(data, status)
        {
            ...
            {
                plUrl = data[getRandomInt(0, data.hls.length)];
            }
        },
        complete: function()
        {
            ...
            if(selectSource())
            {
                var nuSource = (location.search.split('source=')[1]||'').split('&')[0];
                plUrl = "https://"+nuSource;
            }
            createLogoPlayer()
            checkPlUrl();
    ...
    gaTimer = setInterval(function() {
        ga('send', 'event', 'realtime', 'live-page-1m');
    }, 60*1000);
});
...
function createStreamPlayer()
{
    ...
    if(clientPhone)
    {
        streamUrl = "/"+streamName+"-mobile/index.m3u8";
        $("#stream-bandwidth").html("");
    }
    else if(lowQuality)
    {
        streamUrl = "/"+streamName+"-mobile/index.m3u8";
...
//        watermark: "images/ARISS_overlay.png", position: 'bottom-left',
